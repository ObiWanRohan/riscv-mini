...
class AluUtils(width: Int) extends Module {
  // GPT, Reduce
  // Parallel Prefix Adder/Subtractor
  // Barrel Shifter - import verilog blackbox
}

class FastAddSubPipelined(val width: Int) extends Module {
  ...
  io.Result := RegNext(Mux((Sub, ~VecInit(ss).asUInt), VecInit(ss).asUInt))
...
class ReduceNibble extends Module {
  ...
  val gptout2 = GPTInit(
    gptin2.g | (gptin2.p & gptin1.g) | (gptin0.g & gptin1.p & gptin2.p),
    gptin2.p & gptin1.p & gptin0.p,
    gptin2.t
  )

  val gptin3 = io.gptin(3)
  val gptout3 = GPTInit(
    gptin3.g | (gptin3.p & gptin2.g) | (gptin3.p & gptin2.p & gptin1.g) | (gptin0.g & gptin1.p & gptin2.p & gptin3.p),
    gptin3.p & gptin2.p & gptin1.p & gptin0.p,
    gptin3.t
  )
